 ...
 */
/**
 * Entry point for the Count Words application. Demonstrates pluggable rule
 * execution via the WordProcessor pipeline.
 */
public class App {
	...
	public static void main(final String[] args) {
		...
		final List<String> words = List.of("Mango", "apple", "Melon", "kiwi", "Strawberry", "mango", "fig", "Mandarin",
				"blueberry", "Peach");

		log.info("Input words: {}", words);

		// select rules via Factory — no direct instantiation in App
		final List<WordRule<?>> rules = List.of(WordRuleFactory.get(RuleType.STARTS_WITH_M),
		...
		result.getOutput("StartsWithMRule").ifPresent(count -> System.out.println("Words starting with M/m: " + count));

		result.getOutput("LengthGreaterThanFiveRule")
