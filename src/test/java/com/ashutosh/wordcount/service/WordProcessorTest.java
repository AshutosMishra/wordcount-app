...
class WordProcessorTest {
  ...
  void appliesBothRules() {
    ...
    final List<WordRule<?>> rules = List.of(new StartsWithMRule(), new LengthGreaterThanFiveRule());
    ...
    assertEquals(2L, result.getOutput("StartsWithMRule").orElseThrow());
    assertEquals(
        List.of("Strawberry"), result.getOutput("LengthGreaterThanFiveRule").orElseThrow());
  }
  ...
  void throwsOnNullWordsList() {
    final NullPointerException ex =
        assertThrows(NullPointerException.class, () -> processor.process(null, List.of()));
  ...
  void throwsOnNullRulesList() {
    assertThrows(NullPointerException.class, () -> processor.process(List.of("Mango"), null));
  }
  ...
  void throwsOnNullElementInWordsList() {
    ...
    words.add("Mango");
    words.add(null);
    assertThrows(IllegalArgumentException.class, () -> processor.process(words, List.of()));
  }
  ...
  void throwsOnNullElementInRulesList() {
    ...
    rules.add(null);
    assertThrows(IllegalArgumentException.class, () -> processor.process(List.of("Mango"), rules));
  }
}
